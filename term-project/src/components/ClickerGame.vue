...
export default {
  ...
  methods: {
    ...
    autoClick() {
      if (this.data.cps > 0) {
        this.data.totalClicks =
          Math.round((this.data.totalClicks + this.data.cps) * 1e12) / 1e12;
        this.data.money =
          Math.round(
            (this.data.money +
              this.data.clickPower * this.data.clickMulti * this.data.cps) *
              1e12
          ) / 1e12;
      }
    },
    ...
    upgradeClickPower(prop) {
      ...
      this.data.mps =
        Math.round(
          this.data.clickPower * this.data.clickMulti * this.data.cps * 1e12
        ) / 1e12;
    },
...
<template>
  <main id="main-content" class="body-style">
    <section class="page-jumbotron sea-green">
      ...
      <button type="button" class="btn-style margin-y-sm" @click="increment()">
        Click Me
      </button>
      ...
      <article v-if="showUpgrades">
        ...
        <p id="small-clickPower-upgrade" class="margin-y-sm">
          ...
          <span v-if="!purchasedAllUpgrades('SM_CLICK_POWER')" class="">
            (Cost: {{ upgrades.SM_CLICK_POWER.cost }})&nbsp;&nbsp;&nbsp;<button
              type="button"
              class="btn-style margin-y-sm"
              @click="upgradeClickPower('SM_CLICK_POWER')"
              :disabled="!canAfford('SM_CLICK_POWER')"
            >
              Buy
            </button>
          </span>
          ...
          <span v-if="purchasedAllUpgrades('SM_CLICK_POWER')"
            >&nbsp;&nbsp;All upgrades purchased!</span
          >
        </p>
        <p
          id="medium-clickPower-upgrade"
          class="margin-y-sm"
          v-if="revealNewUpgrade('MD_CLICK_POWER')"
        >
          <span>
          ...
          <span v-if="!purchasedAllUpgrades('MD_CLICK_POWER')">
            (Cost: {{ upgrades.MD_CLICK_POWER.cost }})&nbsp;&nbsp;&nbsp;<button
              type="button"
              class="btn-style margin-y-sm"
              @click="upgradeClickPower('MD_CLICK_POWER')"
              :disabled="!canAfford('MD_CLICK_POWER')"
            >
              Buy
            </button>
          </span>
        ...
        <p
          id="large-clickPower-upgrade"
          class="margin-y-sm"
          v-if="revealNewUpgrade('LG_CLICK_POWER')"
        >
          <span>
            <strong>Buy Large Click Power Upgrade:&nbsp;</strong>
            <span class="font-sm"
              >&nbsp;&nbsp;Increase Clicks Power by 100</span
            >
          </span>
          <span v-if="!purchasedAllUpgrades('LG_CLICK_POWER')">
            (Cost: {{ upgrades.LG_CLICK_POWER.cost }})&nbsp;&nbsp;&nbsp;<button
              type="button"
              class="btn-style margin-y-sm"
              @click="upgradeClickPower('LG_CLICK_POWER')"
              :disabled="!canAfford('LG_CLICK_POWER')"
            >
              Buy
            </button>
          </span>
        ...
        <p id="small-autoClicker-upgrade" class="margin-y-sm">
          <span>
            ...
            <span class="font-sm"
              >&nbsp;&nbsp;Increase Clicks per Second (CPS) by 0.1</span
            >
          </span>
          <span v-if="!purchasedAllUpgrades('SM_CPS')"
            >(Cost: {{ upgrades.SM_CPS.cost }})&nbsp;<button
              type="button"
              class="btn-style margin-y-sm"
              @click="upgradeClicksPerSecond('SM_CPS')"
              :disabled="!canAfford('SM_CPS')"
            >
              Buy
            </button>
          </span>
        ...
        <p
          id="medium-autoClicker-upgrade"
          class="margin-y-sm"
          v-if="revealNewUpgrade('MD_CPS')"
        >
          <span>
            <strong>Buy Medium Auto-Clicker&nbsp;</strong>
            <span class="font-sm"
              >&nbsp;&nbsp;Increase Clicks per Second (CPS) by 1</span
            >
          </span>
          <span v-if="!purchasedAllUpgrades('MD_CPS')"
            >(Cost: {{ upgrades.MD_CPS.cost }})&nbsp;<button
              type="button"
              class="btn-style margin-y-sm"
              @click="upgradeClicksPerSecond('MD_CPS')"
              :disabled="!canAfford('MD_CPS')"
            >
              Buy
            </button>
          </span>
        ...
        <p
          id="large-autoClicker-upgrade"
          class="margin-y-sm"
          v-if="revealNewUpgrade('LG_CPS')"
        >
          <span>
            <strong>Buy Large Auto-Clicker&nbsp;</strong>
            <span class="font-sm"
              >&nbsp;&nbsp;Increase Clicks per Second (CPS) by 10</span
            >
          </span>
          <span v-if="!purchasedAllUpgrades('LG_CPS')"
            >(Cost: {{ upgrades.LG_CPS.cost }})&nbsp;<button
              type="button"
              class="btn-style margin-y-sm"
              @click="upgradeClicksPerSecond('LG_CPS')"
              :disabled="!canAfford('LG_CPS')"
            >
              Buy
            </button>
          </span>
