  ...
  methods: {
    submitContactForm(event) {
      ...
      if (this.validateForm()) {
        ...
        hasErrorText.appendChild(
          document.createTextNode(
            "The following errors were detected.  Please modify your values and submit the form again."
          )
        );
    ...
    validateForm() {
      ...
      if (
        this.contactForm.contactName &&
        this.contactForm.contactName.trim() != ""
      ) {
        if (this.contactForm.contactName.length > 2) {
          validName = true;
        } else {
          this.errorMessages.push(
            "ERROR: Contact Name must be greater than 2 characters!"
          );
        }
      } else {
      ...
      if (
        this.contactForm.contactEmail &&
        this.contactForm.contactEmail.trim() != ""
      ) {
        // validate that email matches email pattern (ex. test@gmail.com)
        let emailRegEx = new RegExp("^[\\w!#$%&'*+/=?`{|}~^-]+(?:\\.[\\w!#$%&'*+/=?`{|}~^-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,6}$");
      ...
      if (
        this.contactForm.contactMessage &&
        this.contactForm.contactMessage.trim() != ""
      ) {
        validMessage = true;
      } else {
        this.errorMessages.push(
...
<template>
  <main id="main-content" class="body-style">
    <section class="page-jumbotron sea-green" v-if="!submitted">
      ...
      <p class="text-center">
        If you want to get in touch with me please fill out this contact form so
        that we can connect!
      </p>
      <div
        id="error-message-container"
        class="margin-y-sm text-center red-text"
      ></div>
      <article id="contactMeFormContainer" class="flex-center">
        <h4 hidden>Form Title</h4>
        <form
          name="contactMeFormSubmission"
          method="POST"
          data-netlify="true"
          netlify
        ...
        >
          ...
          <textarea
            ...
            v-model="contactForm.contactMessage"
            required
          ></textarea>

          <div data-netlify-recaptcha="true"></div>
          <div class="flex-center">
            <button type="button" class="btn-style" @click="clearForm">
              Clear Form</button
            >&nbsp;&nbsp;
    ...
    <section id="formSubmissionView" class="page-jumbotron sea-green" v-else>
      ...
      <p class="text-center">
        Thank you for submitting this contact form! Please click the button if
        you would like to submit another form.
      </p>
      <div class="flex-center">
        <button type="button" class="btn-style" @click="newForm">
          New Form
        </button>
      </div>
    </section>
